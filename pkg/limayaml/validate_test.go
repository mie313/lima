...
`
	...
	testCases := []struct {
		...
		{
			name: "ok",
			yaml: fmt.Sprintf(baseYaml, limatype.X8664, limatype.QEMU, "tpm: true"),
		},
		{
			name: "ok - tpm: false",
			yaml: fmt.Sprintf(baseYaml, limatype.X8664, limatype.QEMU, "tpm: false"),
		},
		{
			name: "ok - no tpm field",
		...
		},
		{
			...
			yaml: fmt.Sprintf(baseYaml, limatype.ARMV7L, limatype.QEMU, "tpm: true"),
			wantErrMsg: "currently Windows guest is only supported on [`x86_64` `aarch64`]; got `armv7l`\n" +
				"architecture `armv7l` does not support TPM emulation",
		},
	}
